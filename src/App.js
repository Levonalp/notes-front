...
const getNote = () => {
axios
.get('http://localhost:8000/app/notes')
...
axios
.post('http://localhost:8000/app/notes', addNote)
.then(() => getNote())
}

const handleDelete = event => {
axios
.delete(`http://localhost:8000/app/notes/${event.target.value}`)
.then(() => getNote())
}

const handleUpdate = editNote => {
axios
.put(`http://localhost:8000/app/notes/${editNote.id}`, editNote)
.then(() => getNote())
}
...
return (
  <div>
    ...
    <div className="note container-fluid w-full">
      {note.map(note => (
        <div className="note md-white rounded-md shadow-md flex flex-col text-center m-5" key={note.id}>
